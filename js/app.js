// ============================================================================
// ☁️ CONFIGURAÇÃO DO FIREBASE (NOSSA NUVEM)
// ============================================================================
...
function criarSala() {
    ...
    } else { mostrarAlerta("Digite um nome para a sala!", "Nome Inválido", "fa-triangle-exclamation"); }
}
...
function sairDaSala() {
    ...
    salaAtual = null; perfilAtual = null;
...
function entrarNoSistema() {
    ...
    refSalaAtual.on('value', snapshot => {
        ...
        if (dados.palco && dados.palco.cantor) {
            ...
            else { votosPalcoTemporario = {}; document.getElementById('feed-votos-palco').innerHTML = '<p class="texto-cinza text-center" style="margin: 0; font-style: italic;">Aguardando votos dos jurados...</p>'; }
            ...
            if (!cantorAoVivo || (cantorAoVivo && String(cantorAoVivo.id) !== String(dados.palco.cantor))) {
                ...
                if (perfilCantor && musicaPalco) {
                     ...
                     telaPalcoOverlay.classList.add('escondido'); telaPalcoOverlay.classList.remove('minimizado'); playerVideo.style.pointerEvents = 'auto'; 
                }
            }
            ...
            if (dados.palco.ultimoVoto && dados.palco.ultimoVoto.timestamp > ultimoVotoVisto) {
                ...
                if(voto.nota === 10) { divAlertaPalco.innerHTML = `⭐ NOTA 10 DA PLATEIA! ⭐`; } 
                else { divAlertaPalco.innerHTML = `+${voto.nota} PONTOS!`; }
    ...
    // GATILHO DO TUTORIAL DE BOAS-VINDAS
...
window.onload = () => { 
    ...
    else { document.getElementById('bottom-bar').classList.add('escondido'); }
};

function renderizarFeedVotos(votosObj) {
    const feed = document.getElementById('feed-votos-palco');
    if(!feed) return; feed.innerHTML = '';
    const votos = Object.values(votosObj).sort((a,b) => a.timestamp - b.timestamp);
    
    votos.forEach(voto => {
        let icone = "🌟"; let cor = "white";
        if(voto.nota <= 3) { icone = "💀"; cor = "#ef4444"; }
        else if(voto.nota <= 6) { icone = "😐"; cor = "#f59e0b"; }
        else if(voto.nota <= 9) { icone = "🔥"; cor = "#10b981"; }
        else if(voto.nota === 10) { icone = "🌟"; cor = "gold"; }
        
        const div = document.createElement('div');
        div.style.display = 'flex'; div.style.alignItems = 'center'; div.style.gap = '10px'; div.style.marginBottom = '8px'; div.style.borderBottom = '1px solid rgba(255,255,255,0.05)'; div.style.paddingBottom = '5px';
        div.innerHTML = `<img src="${voto.foto}" style="width: 25px; height: 25px; border-radius: 50%; object-fit: cover;"><div style="flex: 1;"><strong style="color: #38bdf8;">${voto.nome}</strong> <span style="color: ${cor}; font-weight: bold; margin-left: 5px;">deu nota ${voto.nota} ${icone}</span></div>`;
...
function mudarTela(idTelaAlvo, elementoNav = null) {
    ...
    if(elementoNav) { navItems.forEach(item => item.classList.remove('ativo')); elementoNav.classList.add('ativo'); }
...
function entrarComoConvidado() {
    ...
    if (nome !== "") {
        ...
        inputConvidado.value = ''; mostrarAlerta(`Bem-vindo, ${nome}! Você está na sala ${salaAtual}.`, "Visitante", "fa-user-astronaut"); mudarTela('tela-playlist', navItems[1]); 
...
// ============================================================================
// SISTEMA DE TOUR GUIADO (ONBOARDING VIP)
// ============================================================================
...
const passosTour = [
    ...
    { elemento: "nav-catalogo", titulo: "🎵 2. O Catálogo", texto: "Aqui ficam todos os sucessos. Você pode buscar uma música, ouvir a prévia e adicionar o hit na nossa Fila de Espera.", posicao: "acima" },
    { elemento: "nav-fila", titulo: "📋 3. A Fila", texto: "Acompanhe quem será o próximo. Quando chegar a sua vez, é só clicar no botão 'Play' para subir no palco virtual!", posicao: "acima" },
    { elemento: "nav-inicio", titulo: "⭐ 4. Palco e Votação", texto: "Na aba Início você assiste quem está cantando e, o mais legal: pode votar de 1 a 10 para o seu amigo! Boa festa!", posicao: "acima" }
];
...
function renderizarPassoTour() {
    ...
    document.getElementById('tour-titulo').innerText = passo.titulo;
    document.getElementById('tour-texto').innerText = passo.texto;
    ...
    if(passo.elemento) {
        document.getElementById(passo.elemento).classList.add('tour-highlight');
        balao.style.top = 'auto'; balao.style.bottom = '90px'; 
    } else {
        balao.style.bottom = 'auto'; balao.style.top = '30%';
    }
}
...
function renderizarMusicas(musicas) {
    ...
    musicas.forEach(musica => {
        ...
        card.innerHTML = `<div class="info-musica"><div class="titulo-musica">${musica.titulo}</div><div class="artista-musica">${musica.artista}</div></div><div class="botoes-card"><button class="btn-acao btn-previa" id="btn-previa-${musica.id}" onclick="tocarPrevia(${musica.id})" title="Ouvir Prévia"><i class="fa-solid fa-headphones"></i></button><button class="btn-acao btn-fila" onclick="adicionarFila(${musica.id})" title="Add Solo"><i class="fa-solid fa-plus"></i></button><button class="btn-acao btn-dueto" onclick="abrirModalDueto(${musica.id})" title="Add Dueto"><i class="fa-solid fa-user-group"></i></button><button class="btn-acao btn-cantar" onclick="irParaPalco(${musica.id}, null)">Cantar</button></div>`;
        listaMusicas.appendChild(card);
    });
}

function abrirModalDueto(idMusica) {
    if(!perfilAtual || perfilAtual.isGuest) { mostrarAlerta("Identifique-se como Oficial na aba 'Perfil' primeiro!", "Atenção", "fa-user"); mudarTela('tela-perfil', navItems[4]); return; }
...
function adicionarFila(id, parceiro = null) {
    if(!perfilAtual) { mostrarAlerta("Selecione um cantor na aba 'Perfil' primeiro!", "Atenção", "fa-user"); mudarTela('tela-perfil', navItems[4]); return; }
...
function atualizarFilaUI() {
    ...
    filaDeReproducao.forEach((m, index) => {
        ...
        item.innerHTML = `<div class="fila-rank">${index + 1}º</div><div class="avatares-dupla">${htmlAvatares}</div><div class="fila-info-textos"><strong class="fila-nome">${nomeCantor}</strong><span class="fila-musica">${m.titulo}</span></div><div class="fila-acoes"><button class="btn-acao btn-cantar" style="padding: 10px 15px;" onclick="puxarDaFilaParaPalco(${m.instanciaId})"><i class="fa-solid fa-play"></i></button><button class="btn-remover" onclick="removerDaFila(${m.instanciaId})"><i class="fa-solid fa-trash-can"></i></button></div>`;
...
function irParaPalco(idMusica, parceiro = null, pularContagem = false) {
    if(!perfilAtual) { mostrarAlerta("Identifique-se na aba Perfil antes de cantar!", "Atenção", "fa-user"); mudarTela('tela-perfil', navItems[4]); return; }
    ...
    refSalaAtual.child('palco/votos').remove(); refSalaAtual.child('palco/ultimoVoto').remove(); salvarDados(); 
...
function votar(nota) {
    ...
    setTimeout(() => { mudarTela('tela-dashboard', navItems[0]); maximizarPalco(); }, 1500);
}
