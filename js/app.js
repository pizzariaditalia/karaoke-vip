...
let isDJ = false; // Controle de Administrador (DJ)
...
function criarSala() {
    ...
    if (nome !== "") {
        if(salasCriadas.find(s => s.nome.toLowerCase() === nome.toLowerCase())) {
            mostrarAlerta("Já existe uma sala com esse nome!", "Sala já existe", "fa-circle-xmark");
            return;
        }
        salasCriadas.push({ nome: nome, senha: senha !== "" ? senha : null });
        salvarListaSalasGlobais(); 
        
        // Define quem criou a sala como o DJ (Administrador)
...
function entrarNoSistema() {
    ...
    // Verifica se o usuário é o DJ desta sala
    ...
    // Chama a função Premium para escutar as reações voando na tela do palco
    if (typeof escutarReacoesPalco === "function") {
        escutarReacoesPalco(refSalaAtual.child('palco'));
    }
    ...
    refSalaAtual.on('value', snapshot => {
        ...
        if (dados.palco && dados.palco.cantor) {
            ...
            if (dados.palco.votos) {
                votosPalcoTemporario = dados.palco.votos;
                renderizarFeedVotos(votosPalcoTemporario);
            } else {
                votosPalcoTemporario = {};
                const feed = document.getElementById('feed-votos-palco');
                if(feed) feed.innerHTML = '<p class="texto-cinza text-center" style="margin: 0; font-style: italic;">Aguardando votos dos jurados...</p>';
            }
    ...
    // Dispara o Tour VIP Guiado (Se for a primeira vez)
    setTimeout(() => {
...
function renderizarPassoTour() {
    ...
    if(passo.elemento) {
        ...
        balao.style.top = 'auto'; 
        balao.style.bottom = '90px'; 
    } else {
...
function encerrarTour() { 
    document.getElementById('tour-overlay').classList.add('escondido'); 
    document.querySelectorAll('.tour-highlight').forEach(el => el.classList.remove('tour-highlight')); 
    localStorage.setItem('karaoke_tour_visto', 'sim'); 
}
...
function renderizarFavoritos() {
    ...
    if(favoritas.length === 0) {
        container.innerHTML = '<p class="texto-cinza text-center">Nenhuma música favoritada ainda. Vá no Catálogo e clique no coração (🤍) para salvar seu repertório!</p>';
        return;
    }
...
function limparFilaInteiraDJ() {
    if (confirm("Tem certeza que deseja apagar a fila inteira?")) {
        filaDeReproducao = [];
        salvarDados();
    }
}
...
function atualizarFilaUI() {
    ...
    // Adiciona botão Master para o DJ limpar a fila inteira
    if (isDJ) {
        const btnLimpar = document.createElement('button');
        btnLimpar.classList.add('btn-secundario', 'w-100');
        btnLimpar.style.marginBottom = '15px';
        btnLimpar.innerHTML = '<i class="fa-solid fa-trash"></i> Limpar Fila Completa';
        btnLimpar.onclick = limparFilaInteiraDJ;
        listaFila.appendChild(btnLimpar);
    }
...
function irParaPalco(idMusica, parceiro = null, pularContagem = false) {
    ...
    refSalaAtual.child('palco/reacoes').remove(); // Limpa a chuva de emojis da música anterior
